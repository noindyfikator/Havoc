...
public class MainFrame extends java.awt.Frame implements Console.Directory, AWTEventListener {
	...
	setIconImage(icon);
		...
		private UIPanel renderer() {
			...
			switch (rendererType) {
				...
				case "lwjgl":
					System.out.println("Using Experimental Rendering Library");
					return new LWJGLPanel();
				default:
					throw new RuntimeException("Invalid renderer specified: " + rendererType);
			}
		}
	...
	try {
		...
		try {
		    ui.join(5000);
		} catch(InterruptedException e) {CrashLogger.reportCrash(MainFrame.username, Config.clientVersion, Arrays.toString(e.getStackTrace()), true);}
		if(ui.isAlive())
